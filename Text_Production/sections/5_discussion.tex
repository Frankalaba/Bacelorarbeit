...
This growth behavior of \acrshort{thau} makes it more difficult to analyze and, in particular, to transform, as the \acrlong{mbrevi} culture should grow within two to three days for successful transformation. Thus, \acrshort{thau} does not appear to be suitable for basic research due to its unreliable growth.
...
For \acrshort{deu} (see \cref{fig:pred_red_sze_deu}), it can even be concluded that the 5-year model, predicting a lower methane emission reduction than the other two models (23.5~Tg to 26.5~Tg~\ch{CH4}), is likely the most plausible. This can be explained by the fact that in \acrshort{deu}, the energy and waste sectors have remained largely unchanged over the past six years of the dataset (2018--2024), and since emissions are already at a low level, there is little room for further reduction (see \cref{fig:sectors_emission}). From 1990 to 2018, \ch{CH4} emissions from these two sectors decreased continuously. This is still factored into the 10- and 15-year models, leading to predictions with a stronger decrease.
